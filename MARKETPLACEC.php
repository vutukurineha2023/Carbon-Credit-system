...
<head>
    ...
    <style>
        ...
        .popup-content { background:white; padding:20px 30px; border-radius:10px; text-align:center; box-shadow:0 0 15px rgba(0,0,0,0.3); font-size:18px; }
        .popup-content button { margin-top:10px; padding:8px 16px; border:none; background:#28a745; color:white; border-radius:5px; cursor:pointer; }
...
function showPopup(msg) {
    popupMsg.innerHTML = msg + '<br><br><button onclick="closePopup()"><?= t("ok") ?></button>';
...
function undoDelete(id,row,btn) {
    ...
    .then(data=>{
        if(data.success){
            ...
            row.querySelector("td:nth-child(5)").innerText="<?= t('active') ?>";
...
function confirmDelete(id,row,btn) {
    popupMsg.innerHTML = `🗑️ <?= t('confirm_delete') ?><br><br>
        <button id="confirm-delete"><?= t('yes') ?></button>
        <button onclick="closePopup()"><?= t('no') ?></button>`;
    popup.style.display="flex";
    ...
    document.getElementById("confirm-delete").onclick = () => {
        ...
        .then(data=>{
            if(data.success){
                ...
                row.querySelector("td:nth-child(5)").innerText="<?= t('deleted') ?>";
